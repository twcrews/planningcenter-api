...
public class ItemTimeTests(ServicesFixture fixture) : ServicesTestBase(fixture)
{
	[Fact]
	public async Task ItemTime_GetCollectionAsync_ReturnsCollection()
	{
		string? itemId = null;
		...
		try
		{
			...
			var result = await Org.ServiceTypes.WithId(Fixture.ServiceTypeId)
				.Plans.WithId(Fixture.PlanId).Items.WithId(itemId).ItemTimes.GetAsync();
