...
- `People` related resource properties have been fixed and are now collection types.

### Changed

- Move `AddPlanningCenterOAuth` extension methods to the `DependencyInjection` namespace for simplicity.
- Change `AddPlanningCenterOAuth` extension methods to use the `PlanningCenter:ClientId` and `PlanningCenter:ClientSecret` configuration sections by default.
