...
public class BlockoutDateTests(ServicesFixture fixture) : ServicesTestBase(fixture)
{
	[Fact]
	public async Task BlockoutDate_GetCollectionAsync_ReturnsCollection()
	{
		...
		try
		{
			...
			var result = await Org.People.WithId(personId).Blockouts
				.WithId(blockoutId).BlockoutDates.GetAsync();
